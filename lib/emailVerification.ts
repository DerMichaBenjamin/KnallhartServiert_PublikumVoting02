...
export function hashVerificationToken(token: string) {
  return createHash('sha256').update(`${token}.${env('VOTE_VERIFY_SECRET')}`).digest('hex');
}
...
export function buildVerificationUrl(token: string, fallbackOrigin = '') {
  const base = (env('NEXT_PUBLIC_APP_URL') || fallbackOrigin).replace(/\/$/, '');
  if (!base) return `/release-voting/verify?token=${encodeURIComponent(token)}`;
  return `${base}/release-voting/verify?token=${encodeURIComponent(token)}`;
}
