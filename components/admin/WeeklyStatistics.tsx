...
export default function WeeklyStatistics({ stats }: { stats: ReleaseWeekStatistics }) {
  ...
  const comparisonRows = useMemo(() => [...stats.comparisonRows].sort((left, right) => {
    const leftValue = left.audienceAverage ?? Number.NEGATIVE_INFINITY;
    const rightValue = right.audienceAverage ?? Number.NEGATIVE_INFINITY;
  ...
  return <div className="ks-week-statistics" id="weekly-statistics">
    ...
    <section className="ks-card ks-stat-section" id="audience-jury">
      ...
      <div className="ks-table-scroll"><table className="ks-table stats-comparison"><thead><tr><th>Gesamt</th><th>Song</th><th>Künstler</th><th>Publikum</th><th>Jury</th><th className="ks-sortable-th" aria-sort={averageSort === 'desc' ? 'descending' : 'ascending'}><button type="button" className="active" onClick={() => setAverageSort((current) => current === 'desc' ? 'asc' : 'desc')}>Ø Publikum <span aria-hidden="true">{averageSort === 'desc' ? '↓' : '↑'}</span></button></th><th>Differenz</th><th>Publikumsnennungen</th></tr></thead><tbody>{comparisonRows.map((row) => <tr key={row.song.id}><td><span className="ks-rank-pill">{rankLabel(row.overallRank)}</span></td><td><strong>{row.song.title}</strong></td><td>{row.song.artist}</td><td><strong>{row.audienceRank === null ? '—' : `${rankLabel(row.audienceRank)} · ${pointsLabel(row.audiencePoints)}`}</strong></td><td><strong>{row.juryRank === null ? '—' : `${rankLabel(row.juryRank)} · ${pointsLabel(row.juryPoints)}`}</strong></td><td><strong>{row.audienceAverage === null ? '—' : `Ø ${oneDecimal(row.audienceAverage)}`}</strong></td><td><span className={`ks-rank-difference ${(row.rankDifference || 0) > 0 ? 'audience' : (row.rankDifference || 0) < 0 ? 'jury' : ''}`}>{differenceLabel(row.rankDifference)}</span></td><td>{row.audienceMentions}</td></tr>)}</tbody></table></div>
    </section>
