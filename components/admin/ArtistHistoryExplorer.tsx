...
export default function ArtistHistoryExplorer({ artists }: { artists: ArtistHistory[] }) {
  ...
  return <section className="ks-card ks-stat-section" id="artist-history">
    <div className="ks-section-heading"><div><span className="ks-section-kicker">Historische Auswertung</span><h2>Künstlerhistorie und Vergleich</h2><p>Bis zu drei Künstlerangaben aus tatsächlich durchgeführten Votings auswählen und vergleichen.</p></div></div>
    ...
    {!selected.length && <div className="ks-artist-empty"><strong>{artists.length} Künstlerangaben verfügbar</strong><p>Beginne oben mit der Suche. Unterschiedliche Schreibweisen werden bewusst nicht automatisch zusammengeführt, damit keine Künstler fälschlich vermischt werden.</p></div>}
    {selected.length > 0 && <>
