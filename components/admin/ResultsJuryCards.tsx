...
import { buildJuryZonkResults } from '@/lib/zonkResults';
...
export default function ResultsJuryCards({ songs, summary, juryData }: { songs: Song[]; summary: AdminRoundSummary; juryData: AdminJuryRoundData }) {
  ...
  return <section className="ks-card">
    <div className="ks-section-heading"><div><span className="ks-section-kicker">Jury-Voting</span><h2>Einzelne Wertungen</h2><p>Jede persönliche Jury-Wertung und das Publikum als gleichwertige 12-bis-1-Punkte-Stimme.</p></div></div>
    <div className="ks-jury-card-scroll">{cards.map((card) => <article className="ks-jury-ranking-card" key={card.id}><header><div className="ks-avatar">{card.name.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase()}</div><div><strong>{card.name}</strong><small>{card.id === 'audience' ? `${summary.countedVotes} gewertete Stimmen` : card.open ? 'Noch offen' : `Abgegeben · ${formatAdminDateTime(card.submittedAt)}`}</small></div></header>{card.open ? <div className="ks-card-open-state">Noch keine abgeschlossene Wertung.</div> : <ol>{card.rows.map((row) => <li key={row.song.id}><span className="rank">#{row.rank}</span><span className="song"><strong>{row.song.title}</strong><small>{row.song.artist}</small></span><b>{row.points} Pkt.</b></li>)}</ol>}</article>)}</div>

    <div className="ks-section-heading"><div><span className="ks-section-kicker">Jury-ZONK</span><h3>ZONK-Auswertung der Jury</h3><p>{juryZonkVotes ? `${juryZonkVotes} Jury-ZONK-Stimmen wurden abgegeben.` : 'Noch keine Jury-ZONK-Stimme vorhanden.'}</p></div></div>
