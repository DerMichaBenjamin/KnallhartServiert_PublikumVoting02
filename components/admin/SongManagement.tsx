  ...
  return <section className="ks-card" id="songs">
    ...
    <div className="ks-table-scroll">
      <table className="ks-table songs">
        <thead><tr><th>Gesamt</th><th>Song</th><th>Künstler</th><th className="ks-sortable-th" aria-sort={sort.key === 'audience' ? (sort.direction === 'desc' ? 'descending' : 'ascending') : 'none'}>{sortLabel('Publikum', 'audience')}</th><th className="ks-sortable-th" aria-sort={sort.key === 'audienceAverage' ? (sort.direction === 'desc' ? 'descending' : 'ascending') : 'none'}>{sortLabel('Ø Publikum', 'audienceAverage')}</th><th>Status</th><th>Aktionen</th></tr></thead>
        <tbody>
          {rows.map((row) => { const audience = audienceBySong.get(row.song.id); const audienceLeaderboard = audienceLeaderboardBySong.get(row.song.id); return <tr key={row.song.id}><td><strong>{rankLabel(row.rank)}</strong></td><td><strong>{row.song.title}</strong></td><td>{row.song.artist || '—'}</td><td>{summary.countedVotes > 0 ? <strong>{audience ? `${rankLabel(audience.rank)} · ${pointsLabel(row.audiencePoints)}` : `— · ${pointsLabel(0)}`}</strong> : '—'}</td><td><strong>{audienceAverageLabel(summary.countedVotes > 0 ? (audienceLeaderboard?.avg ?? 0) : null)}</strong></td><td><StatusBadge status="success">In Wertung</StatusBadge></td><td><button className="ks-button small warning" type="button" onClick={() => setSongActive(row.song, false)}>Deaktivieren</button></td></tr>; })}
    ...
    <p className="ks-table-footnote"><strong>Ø Publikum</strong> = Summe der Punkte aus allen gewerteten Publikumsstimmen für den Song ÷ Anzahl der gewerteten Publikumsstimmen. Wird ein Song in einer Stimme nicht in die Top 12 gewählt, zählt diese Stimme für den Song mit 0 Punkten.</p>
