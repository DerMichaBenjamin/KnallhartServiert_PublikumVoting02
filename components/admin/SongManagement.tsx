...
type SortKey = 'audience' | 'audienceAverage';
...
function audienceVoteMeta(value: number | null, selectedVotes: number, totalVotes: number) {
  if (value === null || totalVotes <= 0) return '—';
  const voteWord = totalVotes === 1 ? 'Stimme' : 'Stimmen';
  return `${audienceAverageLabel(value)} · ${selectedVotes} von ${totalVotes} ${voteWord}`;
}
...
export default function SongManagement({ round, songs, summary, juryData, post }: { round: Round; songs: Song[]; summary: AdminRoundSummary; juryData: AdminJuryRoundData; post: Post }) {
  const [sort, setSort] = useState<SortState>({ key: 'audienceAverage', direction: 'desc' });
  ...
  const audienceLeaderboardBySong = useMemo(() => new Map(summary.leaderboard.map((row) => [row.song.id, row])), [summary.leaderboard]);
  const rows = useMemo(() => [...results.overallRows].sort((left, right) => {
    const leftValue = sort.key === 'audience' ? left.audiencePoints : (audienceLeaderboardBySong.get(left.song.id)?.avg ?? Number.NEGATIVE_INFINITY);
    const rightValue = sort.key === 'audience' ? right.audiencePoints : (audienceLeaderboardBySong.get(right.song.id)?.avg ?? Number.NEGATIVE_INFINITY);
    const delta = leftValue - rightValue;
    if (delta) return sort.direction === 'asc' ? delta : -delta;
    return (left.rank ?? Number.MAX_SAFE_INTEGER) - (right.rank ?? Number.MAX_SAFE_INTEGER)
      || left.song.title.localeCompare(right.song.title, 'de');
  }), [audienceLeaderboardBySong, results.overallRows, sort]);
  ...
  return <section className="ks-card" id="songs">
    ...
    <div className="ks-table-scroll">
      <table className="ks-table songs">
        <thead><tr><th>Gesamt</th><th>Song</th><th>Künstler</th><th className="ks-sortable-th" aria-sort={sort.key === 'audience' || sort.key === 'audienceAverage' ? (sort.direction === 'desc' ? 'descending' : 'ascending') : 'none'}><div>{sortLabel('Publikum', 'audience')}</div><small>{sortLabel('nach Ø sortieren', 'audienceAverage')}</small></th><th>Status</th><th>Aktionen</th></tr></thead>
        <tbody>
          {rows.map((row) => { const audience = audienceBySong.get(row.song.id); const audienceLeaderboard = audienceLeaderboardBySong.get(row.song.id); return <tr key={row.song.id}><td><strong>{rankLabel(row.rank)}</strong></td><td><strong>{row.song.title}</strong></td><td>{row.song.artist || '—'}</td><td>{summary.countedVotes > 0 ? <><strong>{audience ? `${rankLabel(audience.rank)} · ${pointsLabel(row.audiencePoints)}` : `— · ${pointsLabel(0)}`}</strong><small>{audienceVoteMeta(audienceLeaderboard?.avg ?? 0, audienceLeaderboard?.count ?? 0, summary.countedVotes)}</small></> : '—'}</td><td><StatusBadge status="success">In Wertung</StatusBadge></td><td><button className="ks-button small warning" type="button" onClick={() => setSongActive(row.song, false)}>Deaktivieren</button></td></tr>; })}
          {inactiveSongs.map((song) => <tr key={song.id} className="ks-song-row-inactive"><td>—</td><td><strong>{song.title}</strong></td><td>{song.artist || '—'}</td><td>—</td><td><StatusBadge status="muted">Deaktiviert</StatusBadge></td><td><button className="ks-button small success" type="button" onClick={() => setSongActive(song, true)}>Aktivieren</button></td></tr>)}
          {!songs.length && <tr><td colSpan={6} className="ks-table-empty">Keine Songs vorhanden.</td></tr>}
        </tbody>
      </table>
    </div>
    ...
    <p className="ks-table-footnote"><strong>Ø Publikum</strong> = Summe der Punkte aus allen gewerteten Publikumsstimmen für den Song ÷ Anzahl aller gewerteten Publikumsstimmen. <strong>„64 von 120 Stimmen“</strong> bedeutet z. B., dass 64 Personen den Song in ihre Top 12 gewählt haben. Die übrigen 56 Stimmen zählen für diesen Song beim Durchschnitt mit 0 Punkten.</p>
